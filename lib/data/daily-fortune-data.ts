// 십성별 운세 정보 타입 정의
export interface TenGodFortuneDetail {
  score: number;
  detail: string;
  tip: string;
}

export interface TenGodFortune {
  score: number;
  money: TenGodFortuneDetail;
  love: TenGodFortuneDetail;
  work: TenGodFortuneDetail;
  health: TenGodFortuneDetail;
...
// 오행별 색상
export const elementColors: Record<string, ElementColor> = {
...
// 오행별 행운의 숫자
export const elementNumbers: Record<string, number[]> = {
  '목': [3, 8],
  '화': [2, 7],
  '토': [5, 10],
  '금': [4, 9],
...
// 오행별 방향
export const elementDirections: Record<string, string> = {
  '목': '동쪽',
  '화': '남쪽',
  '토': '중앙',
  '금': '서쪽',
...
// 오행별 음식
export const elementFoods: Record<string, string[]> = {
  '목': ['푸른 채소', '신맛 음식', '닭고기'],
  '화': ['쓴맛 음식', '양고기', '커피'],
  '토': ['단맛 음식', '소고기', '곡물'],
  '금': ['매운 음식', '생선', '무 요리'],
...
export const tenGodPredictions: Record<string, string[]> = {
  '비견': [
...
export const tenGodFortunes: Record<string, TenGodFortune> = {
  '비견': {
    score: 70,
...
// 날짜와 생년월일 기반으로 예언 선택
export function getPrediction(tenGod: string, birthYear: number, birthMonth: number, birthDay: number): string {
...
// 오행 상생상극 관계
export function getElementRelation(userElement: string, todayElement: string): ElementRelation {
...
// 띠 궁합 계산
export function getZodiacCompatibility(birthYear: number): ZodiacCompatibility {
